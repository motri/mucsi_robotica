...
class Robot:
    ...
    def calcular_valores_mas_altos(self):
        ...
        for pos, numbers in enumerate(transposed):
            highest = max(numbers)  # Obtener el valor más alto
            highest_values.append(highest)
            rospy.loginfo(f"Posición {pos + 1}, valores: {numbers}, más alto: {highest}")

        return highest_values
    
    def revision_cartas(self, cartas_detectadas, cartas_defecto = [6,5,8,6]):
